...
fn build_params() -> Result<NoiseParams, NoiseError> {
    NOISE_PROTOCOL_NAME
        .parse()
    ...
    W: AsyncWrite + Unpin,
{
    ...
    state.into_transport_mode().map_err(|err| {
        NoiseError::Protocol(format!("failed to initialize Noise transport state: {err}"))
    })
}
    ...
    W: AsyncWrite + Unpin,
{
    ...
    let mut state = snow::Builder::new(params)
        .psk(0, &derive_psk(token))
        .build_responder()
        .map_err(|err| NoiseError::Protocol(format!("failed to build Noise responder: {err}")))?;

    let frame = match read_frame(reader).await? {
        Some(frame) => frame,
        None => {
            return Err(NoiseError::Protocol(
                "client closed connection during Noise handshake".into(),
            ));
        }
    };
    state
